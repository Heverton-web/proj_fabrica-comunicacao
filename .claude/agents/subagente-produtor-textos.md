...
description: Unidade de fan-out paralelo que produz o material "textos" (WhatsApp, Instagram, LinkedIn) de ponta a ponta — redator-textos → salvar .txt → validar-textos.py → auto-registro em orquestrar-pool-materiais.py. Despachado por /produzir-comunicacao-completa dentro de um lote.
model: inherit
---
...
- `<pasta>` — pasta de destino em `output/<slug>/` (opcional; default `"textos"`). Só é
  ...
  via `orquestrar-pool-materiais.py --proxima-pasta textos` (ex.: `"textos-v2"`, porque já existem

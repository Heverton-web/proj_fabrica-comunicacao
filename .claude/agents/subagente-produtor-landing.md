...
description: Unidade de fan-out paralelo que produz o material "landing-page" de ponta a ponta — redator-landing → compilador-html → validar-html.py → auto-registro em orquestrar-pool-materiais.py. Despachado por /produzir-comunicacao-completa dentro de um lote.
model: inherit
---
...
- `<pasta>` — pasta de destino em `output/<slug>/` (opcional; default `"landing-page"`).
  ...
  regenerada via `orquestrar-pool-materiais.py --proxima-pasta landing-page` (ex.:
